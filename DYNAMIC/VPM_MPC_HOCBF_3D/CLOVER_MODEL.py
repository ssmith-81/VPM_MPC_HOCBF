...
def export_clover_model():
	...
	z_obs = SX.sym('z_obs')
	vz_obs = SX.sym('vz_obs')
	az_obs = SX.sym('az_obs')
	r_obs = SX.sym('r_obs')


	# Parameters (CasADi variable descibing parameters of the DAE; Default [].
	# parameters
	p = vertcat(x_obs, vx_obs, ax_obs, y_obs, vy_obs, ay_obs,z_obs, vz_obs, az_obs, r_obs) # Obstacle state and radius (major axis of ellipsoid estimation)
